...
4. Open the existing EchoScribe item in the Developer Dashboard and upload `artifacts/echoscribe-web-summary-chrome-v1.0.0.zip` as its replacement package. The retained public manifest key preserves the previous unpacked/enterprise ID; the store item remains the authority for store-distributed updates.
