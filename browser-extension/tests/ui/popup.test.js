...
function setup(response = { ok: true, summary: 'New summary', provider: 'openai', model: 'gpt-test' }, options = {}) {
  ...
  const api = {
    ...
    runtime: { sendMessage: vi.fn(async () => response), openOptionsPage: vi.fn(async () => {}) }
  };
...
describe('popup', () => {
  ...
  it('surfaces failures, supports retry, copy, and settings navigation', async () => {
    ...
    api.runtime.sendMessage.mockResolvedValueOnce({ ok: true, summary: 'Retried', provider: 'openai', model: 'gpt-test' });
