...
class CrossAudioPlayer {
  ...
  Future<void> playBytes(Uint8List bytes, {String? mimeType}) async {
    try {
      ...
      debugPrint(
        'CrossAudioPlayer: wrote ${bytes.length} bytes to ${file.path}',
      );
      await _player.play(DeviceFileSource(file.path));
    } catch (e, st) {
      debugPrint('CrossAudioPlayer error: $e\n$st');
      _controller.add(false);
      rethrow;
    }
  }
  ...
  Future<void> pause() async {
    try {
      ...
      debugPrint('CrossAudioPlayer pause error: $e');
    }
  }
  ...
  Future<void> resume() async {
    try {
      ...
      debugPrint('CrossAudioPlayer resume error: $e');
    }
  }
  ...
  Future<void> stop() async {
    try {
      ...
      debugPrint('CrossAudioPlayer stop error: $e');
    } finally {
      _controller.add(false);
    }
  }
