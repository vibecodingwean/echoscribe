...
Install and start EchoScribe Local Whisper.

Options:
  --whisper                  Install/start the local Faster-Whisper server.
  --no-whisper               Skip Local Whisper setup.
...
if [ "$install_whisper" != "yes" ]; then
  echo "Local Whisper setup skipped."
  exit 0
fi
...
echo "=========================="
echo "Server files: $root"
echo "Python venv:  $venv"
echo "Logs:         $logs"
echo "Health URL:   http://127.0.0.1:${whisper_port}/health"
echo "Start:        systemctl --user start echoscribe-local-whisper.service"
echo "Stop:         systemctl --user stop echoscribe-local-whisper.service"
echo "Logs:         journalctl --user -u echoscribe-local-whisper.service -n 80"
